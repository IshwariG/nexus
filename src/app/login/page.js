      ...
      <div style={{ background:'#fff', borderRadius:'16px', padding:'2rem 2.25rem', width:'100%', maxWidth:'460px', boxShadow:'0 20px 60px rgba(0,0,0,0.15)', animation:'fadeIn 0.25s ease' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0.75rem', marginBottom:'1.5rem' }}>
          {[
            ...
            <div key={c.label} style={{ background:'var(--admin-bg)', borderRadius:'8px', padding:'0.7rem 0.9rem', border:'1px solid #f1f3f5' }}>
...
export default function LoginPage() {
  ...
  const goForm = (r) => { setRole(r || selectedRole); setView('form'); setErrorMsg(''); };
  const goBack = () => { setView('selection'); setRole(null); setShowPw(false); setErrorMsg(''); };
      ...
      <div style={s.right}>
        ...
        {view === 'form' && meta && (
          <div style={{ animation:'fadeIn 0.3s ease' }}>
            ...
            {/* Form */}
            {errorMsg && (
              <div style={{ background: '#fee2e2', color: '#dc2626', padding: '0.75rem', borderRadius: '8px', fontSize: '0.8rem', marginBottom: '1.25rem', border: '1px solid #fca5a5', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', margin: '1.25rem 0 1rem 0' }}>
              ...
              <span style={{ padding: '0 0.75rem', fontSize: '0.7rem', color: '#9ca3af', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '1px' }}>or</span>
              <div style={{ flex: 1, height: '1px', background: '#e5e7eb' }}></div>
            </div>
            ...
            <button
              ...
              onMouseEnter={e => { if (!loading) e.currentTarget.style.background = 'var(--admin-surface)'; }}
